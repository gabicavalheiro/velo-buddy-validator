...
const Index = () => {
  ...
  const handleFileSelect = async (file: File) => {
    ...
    try {
      ...
      setResult({
        ...
        cellErrors: [],
        cellWarnings: [], // sempre inicializar o array
        rowCount: 0,
      });
      setActiveTab('dashboard');
    } finally {
  ...
  // Apenas erros bloqueiam — alertas (warnings) não entram neste contador
  const totalErrors = result
    ? result.columnErrors.length + result.cellErrors.reduce((sum, e) => sum + e.failCount, 0)
    : 0;

  const totalWarnings = result?.cellWarnings?.length ?? 0;
        ...
        {result?.success && !loading && (
          ...
          >
            ...
            <p className="text-muted-foreground">
              As colunas e as células têm a formatação exata. Podes importar sem medo!
            </p>
            <p className="mt-2 text-sm text-muted-foreground">
              {result.rowCount} linhas analisadas em <strong>{fileName}</strong>
            </p>
            {totalWarnings > 0 && (
              <p className="mt-2 text-sm text-yellow-600 font-medium">
                ⚠️ {totalWarnings} alerta{totalWarnings > 1 ? 's' : ''} de endereço — consulta o Dashboard para detalhes
              </p>
            )}
          </motion.div>
        )}
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="w-full grid grid-cols-2 h-12 rounded-xl bg-muted p-1 mb-6">
            ...
            <TabsTrigger value="dashboard" className="rounded-lg text-sm font-semibold gap-2 data-[state=active]:shadow-soft relative">
              ...
              {/* Badge vermelho — erros bloqueantes */}
              ...
              {/* Badge amarelo — alertas não bloqueantes (só aparece se não há erros) */}
              {result && result.success && totalWarnings > 0 && (
                <span className="absolute -top-1 -right-1 h-5 min-w-[20px] flex items-center justify-center rounded-full bg-yellow-400 text-yellow-900 text-[10px] font-bold px-1">
                  {totalWarnings}
                </span>
              )}
            </TabsTrigger>
